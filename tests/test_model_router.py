...
def test_router_falls_through_after_non_transient_failures():
    """404/402 errors move immediately to the next provider."""
...
def test_router_retries_transient_errors_before_falling_through():
    """429 is transient — router retries 3 times before moving on."""
    ...
    assert result["provider"] == "groq"
    assert gemini.complete.call_count == 3  # retried 3 times
    groq.complete.assert_called_once()


def test_router_raises_when_all_providers_exhausted():
    """RuntimeError raised when every provider in the chain fails."""
    providers = []
